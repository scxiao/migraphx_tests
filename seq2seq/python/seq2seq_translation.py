...
def evaluate(encoder, decoder, input_lang, output_lang, hidden_size, max_sent_len, sent, target_name):
    ...
    for i in range(input_len):
        ...
        for key, value in encoder.get_parameter_shapes().items():
            ...
            if target_name == "gpu":
                encoder_params[key]  = migraphx.to_gpu(migraphx.argument(args[-1]))
            else:
                encoder_params[key]  = migraphx.argument(args[-1])
        
        if target_name == "gpu":
            result = np.array(migraphx.from_gpu(encoder.run(encoder_params)))
        else:
            result = np.array(encoder.run(encoder_params))

        encoder_hidden = result.reshape(-1).tolist()
        encoder_output.extend(encoder_hidden)
    ...
    for i in range(max_sent_len):
        ...
        for key, value in decoder.get_parameter_shapes().items():
            if key == "input.1":
                args.append(np.array(decoder_input).astype(np.longlong).reshape(value.lens()))
            elif key == "hidden":
            ...
            if target_name == "gpu":
                params[key] = migraphx.to_gpu(migraphx.argument(args[-1]))
            else:
                params[key] = migraphx.argument(args[-1])

        if target_name == "gpu":
            result = np.array(migraphx.from_gpu(decoder.run(params)))
        else:
            result = np.array(decoder.run(params))
            
        outputs = result.reshape(-1).tolist()
        decoder_output = outputs[0:output_lang.n_words]
        decoder_hidden = outputs[output_lang.n_words:]
...
def main():
    ...
    input_lang, output_lang, pairs = readLangs(sys.argv[3], sys.argv[4], False)
    for pair in pairs:
