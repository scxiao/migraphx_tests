...
def main():
    if len(sys.argv) != 4:
        print("Usage: python onnxruntime_mrpc.py file.onnx input.tsv ep_name (CUDA, Tensorrt, MIGraphX, CPU(Default))")
        exit(1)
    ...
    ep_name = sys.argv[3] + 'ExecutionProvider'
    ...
    # read input file
    lines = open(sys.argv[2], encoding='utf8').read().strip().split('\n')
