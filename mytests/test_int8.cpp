...
{
    migraphx::program p;
    migraphx::shape m1_shape{migraphx::shape::int8_type, {6, 3}};
    migraphx::shape m2_shape{migraphx::shape::int8_type, {6, 5}};
    migraphx::shape m3_shape{migraphx::shape::int32_type, {3, 5}};
    std::vector<int8_t> data1(3 * 6);
    std::vector<int8_t> data2(6 * 5);
    ...
    auto r = mm->add_instruction(migraphx::op::quant_dot{alpha, beta}, tl1, l2);
    mm->add_return({r});

    return p;
}

migraphx::program create_trans()
{
    migraphx::program p;
    auto* mm = p.get_main_module();
...
int main(int argc, char **argv) {
    std::vector<std::vector<int32_t>> cpu_res, gpu_res; 
    auto p = create_trans();
    run_options ref_options;
    ref_options.t = migraphx::ref::target{};
    run_prog(p, cpu_res, ref_options);
    run_options gpu_options;
    gpu_options.t = migraphx::gpu::target{};
    run_prog(p, gpu_res, gpu_options);

    bool res = compare_results(cpu_res[0], gpu_res[0]);
