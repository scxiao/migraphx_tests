...
std::string jsonize_object_string(const std::string& str)
{
    if (str.empty())
    {
        return {};
    ...
    {
        // processing key, add quote before and after the key
        auto c_pos = str.find_first_not_of(':', pos);
        if (c_pos == std::string::npos)
        {
            std::abort();
        }

        auto key_str = str.substr(pos, c_pos);
        result.append(1, '{');
        result(add_quote(key_str));
        result.append(": ");
        ...
        else
        {
            ...
            if (str[sp] == '{')
            {
                ...
                auto json_obj_str = jsonize_object_string(obj_str);
            ...
            {
                auto cp = str.first_first_of(',', pos);
